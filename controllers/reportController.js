...
import { createPDFReport,  createPDFSummary, createPDFGoalProgressReport  } from "../services/createPDFReportService.js";
...
export const generateGoalProgressReport = async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        if (format === "json") {
            return res.status(StatusCodes.OK).json({
                msg: "Goal progress report generated successfully",
                data: goalsWithProgress,
            });
        }

        // Generate PDF report if requested
        const pdfPath = path.join("reports", `goal_progress_report_${Date.now()}.pdf`);
        await createPDFGoalProgressReport(goalsWithProgress, pdfPath);

        // Send PDF as response
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename=goal_progress_report_${Date.now()}.pdf`);
        fs.createReadStream(pdfPath).pipe(res);

    } catch (error) {
        console.error("Error generating goal progress report:", error);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            msg: "Error generating goal progress report",
            error: error.message,
        });
    }
};