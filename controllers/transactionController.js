...
export const createTransaction = async (req, res) => {
    try {
        ...
        // Handle income transactions (create or update saving goal)
        if (type === "income") {
            const goalName = "Savings Goal"; // Default goal name for savings
            const goal = await Goal.findOne({ userId: req.user.userId, name: goalName });

            if (!goal) {
                // Create a new saving goal if it doesn't exist
                const newGoal = new Goal({
                    userId: req.user.userId,
                    name: goalName,
                    targetAmount: convertedAmount, // Set initial target to the income amount
                    currentAmount: convertedAmount, // Set current amount to the income amount
                    deadline: new Date(new Date().setFullYear(new Date().getFullYear() + 1)), // Set deadline to 1 year from now
                    status: "In Progress"
                });

                await newGoal.save();
            } else {
                // Update the existing saving goal
