...
export const createPDFGoalProgressReport = async (goals, filePath) => {
    return new Promise(async (resolve, reject) => {
        const doc = new PDFDocument({ margin: 50, size: "A4" });
        const stream = fs.createWriteStream(filePath);
        doc.pipe(stream);
        ...
        doc.fillColor("#2c3e50")
           .fontSize(24)
           .text("Goal Progress Report", { align: "center" });
        doc.moveDown(1);
        ...
        goals.forEach((goal, index) => {
            ...
            doc.fontSize(12)
               .text(`Target Amount: $${goal.targetAmount.toFixed(2)}`);
            doc.text(`Current Amount: $${goal.currentAmount.toFixed(2)}`);
            ...
            const progressWidth = (goal.progress / 100) * progressBarWidth;

            doc.rect(50, doc.y, progressBarWidth, progressBarHeight)
               .stroke("#cccccc");
            doc.rect(50, doc.y, progressWidth, progressBarHeight)
               .fillAndStroke("#2ecc71", "#2ecc71");
            doc.moveDown(2);
        });
